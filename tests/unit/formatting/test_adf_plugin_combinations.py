...
:::"""
        ...
        content_types = {
            block.get("type") for block in expand_content if isinstance(block, dict)
        }
...
:::"""
        ...
        for column in layout["content"]:
            ...
            has_paragraph = any(
                block["type"] == "paragraph" for block in section["content"]
            )
        ...
        for row in table["content"][1:]:
            for cell in row["content"]:
                ...
                if cell["content"] and cell["content"][0]["type"] == "paragraph":
                    content_types = {
                        node.get("type")
                        for node in cell["content"][0]["content"]
