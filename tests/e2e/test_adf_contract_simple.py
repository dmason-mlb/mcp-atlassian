...
class TestADFContractSimple:
    ...
    def test_adf_payload_structure_validation(self, sample_test_data):
        ...
        for element_type, description in expected_adf_elements.items():
            assert element_type in ["heading", "panel", "status", "codeBlock", "date"], \
                ...
                "description": {
                    ...
                    "content": [
                        # ADF content nodes would be here
                        {
                            "type": "heading",
                            "attrs": {"level": 1},
                            "content": [{"type": "text", "text": "Advanced ADF Test Content"}]
                        }
                    ]
                }
            }
    ...
    def test_confluence_page_adf_contract_structure(self, atlassian_stub, sample_test_data):
    ...
    def _validate_jira_issue_payload_structure(self, payload: Dict):
        ...
        if "description" in fields:
            ...
            if isinstance(desc, dict):
                assert desc.get("type") == "doc", "ADF description should be document type"
        ...
        if "body" in payload:
            ...
            if "atlas_doc_format" in body:
                ...
                if isinstance(value, dict):
                    assert value.get("type") == "doc", "ADF value should be document type"
    ...
    def _validate_adf_panel_structure(self, panel_element: Dict):
        ...
        assert attrs["panelType"] in valid_types, f"Invalid panel type: {attrs['panelType']}"
