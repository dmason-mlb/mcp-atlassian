...
class ADFValidator:
    ...
    def validate_adf(self, content: dict | str) -> ValidationResult:
        """
    ...
    def _validate_content_array(
        self, content: list, result: ValidationResult, path: str = ""
    ):
                ...
                for j, cell_node in enumerate(cells):
                    if isinstance(cell_node, dict):
                        ...
                        if cell_type not in ["tableCell", "tableHeader"]:
                            result.add_warning(
                                f"Unexpected cell type '{cell_type}' at {path}[{i}][{j}]"
                            )
    ...
    def _validate_attributes(
        self, attrs: dict, node_type: str, result: ValidationResult, path: str
    ):
    ...
    def _count_nodes(self, content: dict | list, count: int = 0) -> int:
    ...
    def validate_wiki_markup(self, content: str) -> ValidationResult:
        ...
        for indicator in adf_indicators:
            if indicator in content:
                result.add_warning(
                    f"Found ADF-like content in wiki markup: {indicator}"
                )
...
class ResponseValidator:
    ...
    def validate_jira_issue_response(
        ...
        if operation == "create":
            ...
            if "issue" in data and isinstance(data.get("issue"), dict):
                ...
                field_result = self._validate_required_fields(
                    data, required_fields, result
                )
                ...
                missing_issue_fields = [
                    f for f in issue_required if f not in issue_data
                ]
                if missing_issue_fields:
                    issue_result.add_error(
                        f"Missing required fields in issue: {missing_issue_fields}"
                    )
        ...
        for path in description_paths:
            ...
            if desc_content:
                format_result = self._validate_content_format(
                    {"content": desc_content}, deployment_type
                )
                result.merge(format_result)
                break
    ...
    def validate_confluence_page_response(
            ...
            if "page" in data and isinstance(data.get("page"), dict):
                ...
                if missing_page_fields:
                    page_result.add_error(
                        f"Missing required fields in page: {missing_page_fields}"
                    )
        ...
        for path in content_paths:
            ...
            if content:
                format_result = self._validate_content_format(
                    {"content": content}, deployment_type
                )
                result.merge(format_result)
                break
    ...
    def _validate_basic_structure(
        self, data: dict[str, Any], expected_type: str, result: ValidationResult
    ):
        ...
        if expected_type == "error":
            ...
            if not has_error:
                result.add_warning(
                    "Expected error response but no error indicators found"
                )
            ...
            for field in error_fields:
                if field in data and data[field]:
                    result.add_error(
                        f"Unexpected error in success response: {data[field]}"
                    )

    def _validate_required_fields(
        self, data: dict[str, Any], required_fields: list[str], result: ValidationResult
    ) -> ValidationResult:
    ...
    def _validate_content_format(
        self, data: dict[str, Any], deployment_type: DeploymentType
    ) -> ValidationResult:
            ...
            if isinstance(content_value, dict):
                ...
                if self._looks_like_adf_json(content_value):
                    try:
                        ...
                        result.add_warning(
                            "Content looks like ADF JSON but failed to parse"
                        )
                else:
...
def validate_adf(content: dict | str) -> ValidationResult:
...
def validate_response(
    ...
    return validator.validate_response(
        response, expected_type, deployment_type, expected_fields
    )
...
def validate_confluence_response(
    ...
    validator = ResponseValidator()
    return validator.validate_confluence_page_response(
        response, operation, deployment_type
    )
