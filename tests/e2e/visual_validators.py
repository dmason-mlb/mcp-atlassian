...
class ADFVisualValidator:
    ...
    async def validate_jira_issue_description(
        ...
        try:
            ...
            await page.wait_for_selector("[data-testid='issue.views.issue-base.foundation.summary.heading']", timeout=10000)
            ...
            if expected_elements:
                await self._validate_assertions(page, expected_elements, result, description_selector)
            
            # Default validations for common ADF elements
            await self._validate_default_adf_elements(page, result, description_selector)
            
            result.details["issue_key"] = issue_key
            result.details["description_text"] = await description_element.text_content()
            
        except Exception as e:
            result.passed = False
            result.failed_assertions.append(f"Failed to validate Jira issue {issue_key}: {e}")
    ...
    async def validate_confluence_page_content(
        ...
        try:
            ...
            await page.wait_for_selector(".wiki-content, .ak-renderer-document", timeout=10000)
            ...
            # Validate expected elements
            if expected_elements:
                await self._validate_assertions(page, expected_elements, result, content_selector)
            ...
            result.passed = False
            result.failed_assertions.append(f"Failed to validate Confluence page {page_id}: {e}")
    ...
    async def validate_comment_content(
        ...
        try:
            ...
            if expected_elements:
                await self._validate_assertions(page, expected_elements, result, comment_selector)
        ...
        for assertion in assertions:
            try:
                ...
                if assertion.assertion_type == VisualAssertionType.ELEMENT_EXISTS:
                    ...
                    if count == 0:
                        result.failed_assertions.append(f"Element not found: {assertion.description}")
                        result.passed = False
                    ...
                    if not is_visible:
                        result.failed_assertions.append(f"Element not visible: {assertion.description}")
                        result.passed = False
                    ...
                    if await element.count() > 0:
                        ...
                        result.failed_assertions.append(f"Element not found for text check: {assertion.description}")
                        result.passed = False
                    ...
                    if await element.count() > 0:
                        await self._validate_element_styling(element.first(), assertion, result)
                    else:
                        result.failed_assertions.append(f"Element not found for styling check: {assertion.description}")
                        result.passed = False
                
            except Exception as e:
                result.failed_assertions.append(f"Assertion failed: {assertion.description} - {e}")
                result.passed = False
    ...
    async def _validate_default_adf_elements(
        ...
        if panel_count > 0:
            ...
            for i in range(min(panel_count, 3)):  # Check first 3 panels
                ...
                if panel_type:
                    ...
                    bg_color = await panel.evaluate("el => getComputedStyle(el).backgroundColor")
                    if bg_color == "rgba(0, 0, 0, 0)" or bg_color == "transparent":
                        result.failed_assertions.append(f"Panel {i} ({panel_type}) has no background color")
                        result.passed = False
        ...
        if status_count > 0:
            ...
            for i in range(min(status_count, 3)):
                ...
                border_radius = await status.evaluate("el => getComputedStyle(el).borderRadius")
                
                if border_radius == "0px":
                    result.failed_assertions.append(f"Status element {i} should have border radius")
                    result.passed = False
        ...
        if code_count > 0:
            ...
            for i in range(min(code_count, 2)):
                ...
                font_family = await code_block.evaluate("el => getComputedStyle(el).fontFamily")
                
                if "mono" not in font_family.lower():
                    result.failed_assertions.append(f"Code block {i} should use monospace font, got: {font_family}")
                    result.passed = False
    ...
    async def _detect_panel_type(self, panel: Locator) -> Optional[str]:
    ...
    async def _validate_element_styling(
        ...
        if not isinstance(assertion.expected_value, dict):
            result.failed_assertions.append(f"Styling assertion must have dict expected_value: {assertion.description}")
            result.passed = False
            return
        
        for style_property, expected_pattern in assertion.expected_value.items():
            actual_value = await element.evaluate(f"el => getComputedStyle(el).{style_property}")
...
class ADFVisualTestBuilder:
    ...
    def expect_element(self, selector: str, description: str = "") -> 'ADFVisualTestBuilder':
        """Expect element to exist."""
        self.assertions.append(VisualAssertion(
    ...
    def expect_visible(self, selector: str, description: str = "") -> 'ADFVisualTestBuilder':
    ...
    def expect_text(self, selector: str, text: str, description: str = "") -> 'ADFVisualTestBuilder':
        """Expect element to contain text."""
        self.assertions.append(VisualAssertion(
            VisualAssertionType.TEXT_CONTENT,
            selector,
            expected_value=text,
            description=description or f"Element {selector} should contain '{text}'"
        ))
        return self
    
    def expect_count(self, selector: str, count: int, description: str = "") -> 'ADFVisualTestBuilder':
        """Expect specific count of elements."""
        self.assertions.append(VisualAssertion(
            VisualAssertionType.ELEMENT_COUNT,
            selector,
            expected_value=count,
            description=description or f"Should have {count} elements matching {selector}"
        ))
        return self
    
    def expect_styling(self, selector: str, styles: Dict[str, str], description: str = "") -> 'ADFVisualTestBuilder':
        """Expect element to have specific styling."""
        self.assertions.append(VisualAssertion(
            VisualAssertionType.STYLING,
            selector,
            expected_value=styles,
            description=description or f"Element {selector} should have specified styles"
        ))
        return self
    ...
    def expect_panel(self, panel_type: str, text: str = None) -> 'ADFVisualTestBuilder':
        ...
        if text:
            self.expect_text(selector, text, f"{panel_type} panel should contain '{text}'")
        
        return self
    
    def expect_status(self, color: str = None, text: str = None) -> 'ADFVisualTestBuilder':
    ...
    def expect_code_block(self, language: str = None) -> 'ADFVisualTestBuilder':
        ...
        self.expect_styling(selector, {"font-family": r".*mono.*"}, "Code block should use monospace font")
...
async def validate_adf_visually(
    ...
    if content_type == "jira_issue":
        return await validator.validate_jira_issue_description(page, content_id, expected_elements)
    elif content_type == "confluence_page":
        return await validator.validate_confluence_page_content(page, content_id, expected_elements)
    else:
