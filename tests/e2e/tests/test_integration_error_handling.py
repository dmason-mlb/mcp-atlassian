...
class TestCrossServiceIntegration(MCPBaseTest):
    ...
    async def test_linked_content_workflow(self, mcp_client, test_config):
        ...
        link_result = await mcp_client.jira_link_to_epic(
            issue_key=story_key, epic_key=epic_key
        )
...
"""
        ...
        comment_result = await mcp_client.add_jira_comment(
            issue_key=story_key, comment=comment_content
        )
        ...
        epic_details = await mcp_client.jira_get_issue(
            issue_key=epic_key, expand="changelog"
        )
        ...
        story_details = await mcp_client.jira_get_issue(
            issue_key=story_key, expand="changelog"
        )
        ...
        page_details = await mcp_client.confluence_get_page(
            page_id=doc_page_id, include_metadata=True
        )
...
"""
                + "\n".join(
                    [
                        f"- {key}: Bulk test issue {j + 1}"
                        for j, key in enumerate(jira_issues)
                    ]
                ),
                content_format="markdown",
            )
        ...
        for issue_key in jira_issues:
            assert issue_key in found_jira_keys, (
                f"Bulk created issue {issue_key} not found in search"
            )
        ...
        for page_id in confluence_pages:
            assert page_id in found_confluence_ids, (
                f"Bulk created page {page_id} not found in search"
            )
...
"""
        ...
        assert sync_page_id in str(updated_description), (
            "Issue should reference the page"
        )

        updated_page = await mcp_client.confluence_get_page(page_id=sync_page_id)
        updated_page_content = self.extract_value(updated_page, "content") or str(
            updated_page
        )
...
class TestErrorHandlingResilience(MCPBaseTest):
    ...
    async def test_invalid_resource_handling(self, mcp_client, test_config):
        ...
        try:
            invalid_issue_result = await mcp_client.jira_get_issue(
                issue_key=invalid_issue_key
            )
            # If no exception, check for error in response
            if invalid_issue_result:
                self.assert_error_response(
                    invalid_issue_result, r"not found|does not exist|invalid"
                )
        ...
        try:
            invalid_page_result = await mcp_client.confluence_get_page(
                page_id=invalid_page_id
            )
            # If no exception, check for error in response
            if invalid_page_result:
                self.assert_error_response(
                    invalid_page_result, r"not found|does not exist|invalid"
                )
        except Exception as e:
        ...
        try:
            ...
            if invalid_project_result:
                self.assert_error_response(
                    invalid_project_result, r"project|not found|invalid"
                )
        except Exception as e:
            # Exception is expected for invalid project
            assert any(
                term in str(e).lower() for term in ["project", "not found", "invalid"]
            )
    ...
    async def test_malformed_data_handling(self, mcp_client, test_config):
        ...
        try:
            ...
            assert any(
                term in str(e).lower() for term in ["length", "long", "limit", "size"]
            )
...
"""
    ...
    async def test_concurrent_operations_handling(self, mcp_client, test_config):
        ...
        async def update_issue(client, issue_key, update_num):
            try:
                result = await client.jira_update_issue(
                    issue_key=issue_key,
                    fields={
                        "description": f"Updated description {update_num} - concurrent test"
                    },
                )
        ...
        assert successful_comments >= 1, (
            "At least one concurrent comment should succeed"
        )
        ...
        final_issue = await mcp_client.jira_get_issue(
            issue_key=issue_key, expand="changelog"
        )
    ...
    async def test_api_rate_limiting_handling(self, mcp_client, test_config):
            ...
            if i % 2 == 0:
                ...
                rapid_operations.append(
                    mcp_client.confluence_search(
                        query=f"space={test_config['confluence_space']}", limit=1
                    )
                )
        ...
        for operation in rapid_operations:
            try:
                ...
                if any(
                    term in str(e).lower()
                    for term in ["rate", "limit", "throttle", "429"]
                ):
        ...
        rate_limited_operations = [
            r for r in results if isinstance(r, str) and "rate limited" in r.lower()
        ]

        # Should have at least some successful operations
        assert len(successful_operations) >= 1, (
            "Should have at least one successful operation"
        )

        # If rate limited, that's acceptable behavior
        if rate_limited_operations:
            print(
                f"Encountered rate limiting (expected behavior): {len(rate_limited_operations)} operations"
            )
    ...
    async def test_network_resilience_patterns(self, mcp_client, test_config):
        ...
        try:
            ...
            large_content = (
                self.generate_test_description("adf_rich") * 10
            )  # Repeat content
            ...
            if (
                large_content_result
                and "error" not in str(large_content_result).lower()
            ):
            ...
            if any(
                term in str(e).lower()
                for term in ["timeout", "network", "connection", "unreachable"]
            ):
