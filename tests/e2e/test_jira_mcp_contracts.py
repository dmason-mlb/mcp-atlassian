...
class TestJiraMCPContracts:
    ...
    @pytest.mark.asyncio
    async def test_jira_create_issue_mcp_contract(
    ...
    @pytest.mark.asyncio
    async def test_jira_create_issue_with_priority_contract(
        self, mcp_client, atlassian_stub
    ):
        """Test MCP contract for issue creation with priority."""
        project_key = "TEST"

        expected_issue = {"key": "TEST-456", "fields": {"priority": {"name": "High"}}}
        atlassian_stub.stub_jira_create_issue(project_key, expected_issue)
    ...
    @pytest.mark.asyncio
    async def test_jira_create_issue_with_labels_contract(
        self, mcp_client, atlassian_stub
    ):
        ...
        result = await mcp_client.call_tool(
            "jira_create_issue",
            {
                ...
                "issue_type": "Task",
                "additional_fields": {"labels": ["test", "automation", "mcp"]},
            },
        )
    ...
    @pytest.mark.asyncio
    async def test_jira_add_comment_mcp_contract(
        self, mcp_client, atlassian_stub, sample_test_data
    ):
...
```"""
        ...
        result = await mcp_client.call_tool(
            "jira_add_comment", {"issue_key": issue_key, "comment": comment_text}
        )
    ...
    async def test_jira_search_mcp_contract(self, mcp_client, atlassian_stub):
        ...
        expected_search = {
            "issues": [
                ...
                {
                    "key": "TEST-2",
                    "fields": {"summary": "Second Issue", "status": {"name": "To Do"}},
                },
            ],
            "total": 2,
            "startAt": 0,
        ...
        result = await mcp_client.call_tool(
            "jira_search", {"jql": jql, "fields": "summary,status", "limit": 10}
        )
    ...
    @pytest.mark.asyncio
    async def test_jira_create_issue_link_mcp_contract(
        self, mcp_client, atlassian_stub
    ):
