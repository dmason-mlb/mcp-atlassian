...
def adf_extensions(md):
    ...
    md.block.register(
        "layout_block", layout_pattern, parse_layout_block, before="fenced_code"
    )
...
class ADFRenderer(mistune.BaseRenderer):
    ...
    def __init__(
        self,
        validator: ADFValidator | None = None,
        ...
        """
        super().__init__()
        self.validator = validator or ADFValidator(
    ...
    def render_token(
        self, token: dict[str, Any], state: mistune.core.BlockState
    ) -> Any:
    ...
    def render_tokens(
        self, tokens: list[dict[str, Any]], state: mistune.core.BlockState
    ) -> list[Any]:
    ...
    def render_layout_block(
        ...
        data = {"type": "layout", "num_columns": num_columns, "columns": columns}
    ...
    def render_adf_block(
            ...
            if content.strip():
                ...
                for para in paragraphs:
                    if para.strip():
                        ...
                        inline_tokens = (
                            self.inline_parser.parse(inline_state)
        ...
        return {"type": "paragraph", "content": [{"type": "text", "text": raw_text}]}
    ...
    def render_table_cell(
        ...
        cell_type = (
            "tableHeader"
            if is_head or token.get("attrs", {}).get("head")
            else "tableCell"
        )
    ...
    def render_inline_token(
        ...
        if token_type == "text":
            ...
            return {"type": "text", "text": code_text, "marks": [{"type": "code"}]}
    ...
    def _apply_mark(
        ...
        if children:
            ...
            if len(inner_nodes) == 1 and inner_nodes[0].get("type") == "text":
                ...
                if not is_valid:
                    if self.validator.validation_level == ADFValidator.VALIDATION_ERROR:
                        ...
                        self.validator.validation_level == ADFValidator.VALIDATION_WARN
                    ):
            ...
            return {"type": "text", "text": token.get("raw", ""), "marks": [mark]}
    ...
    def _process_inline_plugins(
        ...
        while i < len(nodes):
            if nodes[i].get("type") == "text" and not nodes[i].get("marks"):
                ...
                while (
                    j < len(nodes)
                    and nodes[j].get("type") == "text"
                    and not nodes[j].get("marks")
                ):
                    text_parts.append(nodes[j]["text"])
                    j += 1

                # Create merged node
                merged_nodes.append({"type": "text", "text": "".join(text_parts)})
                i = j
            else:
                ...
                if all_matches:
                    ...
                    for match, plugin in all_matches:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Error processing inline plugin {plugin.name}: {e}"
                            )
                            # Fall back to text
                            result.append({"type": "text", "text": match.group(0)})
...
class ASTBasedADFGenerator:
    ...
    def _create_error_document(
        self, original_text: str, errors: list[str]
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "type": "doc",
            "content": [
                {"type": "paragraph", "content": [{"type": "text", "text": error_text}]}
            ],
        }
