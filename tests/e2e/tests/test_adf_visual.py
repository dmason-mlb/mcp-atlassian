...
class TestADFVisualFormatting(MCPBaseTest):
    ...
    @pytest.mark.visual
    @pytest.mark.adf
    def test_jira_issue_adf_description_rendering(
        self, sync_mcp_client, test_config, context
    ):
...
"""
        ...
        # Create the issue using sync wrapper (runs in separate thread)
        result = sync_mcp_client.create_jira_issue(
        ...
        # For now, just validate that we can create the issue
        # TODO: Add browser-based validation once playwright integration is stable
        assert issue_key is not None, "Issue should be created successfully"
        assert issue_key.startswith(test_config["jira_project"]), (
            f"Issue key should start with project: {issue_key}"
        )

    @pytest.mark.visual
    @pytest.mark.adf
    def test_confluence_page_adf_content_rendering(
        self, sync_mcp_client, test_config, context
    ):
...
"""
        ...
        # Create the page using sync wrapper
        result = sync_mcp_client.create_confluence_page(
        ...
        # For now, just validate that we can create the page
        # TODO: Add browser-based validation once playwright integration is stable
        assert page_id is not None, "Page should be created successfully"

    @pytest.mark.visual
    @pytest.mark.adf
    def test_jira_comment_adf_formatting(self, sync_mcp_client, test_config, context):
        """Test ADF formatting in Jira comments."""
        # First create a test issue
        issue_result = sync_mcp_client.create_jira_issue(
            project_key=test_config["jira_project"],
...
"""
        ...
        comment_result = sync_mcp_client.add_jira_comment(
            issue_key=issue_key, comment=adf_comment
        )

        self.assert_success_response(comment_result)

        # For now, just validate that we can add the comment
        # TODO: Add browser-based validation once playwright integration is stable
        assert comment_result is not None, "Comment should be added successfully"

    @pytest.mark.visual
    @pytest.mark.adf
    def test_cross_platform_adf_consistency(
        self, sync_mcp_client, test_config, context
    ):
...
"""
        ...
        jira_result = sync_mcp_client.create_jira_issue(
            project_key=test_config["jira_project"],
        ...
        confluence_result = sync_mcp_client.create_confluence_page(
            space_id=test_config["confluence_space"],
        ...
        # For now, just validate that we can create both resources
        # TODO: Add browser-based cross-platform validation once playwright integration is stable
        assert issue_key is not None, "Jira issue should be created successfully"
        assert page_id is not None, "Confluence page should be created successfully"

    @pytest.mark.visual
    @pytest.mark.adf
    def test_adf_performance_visual_validation(
        self, sync_mcp_client, test_config, context
    ):
...
:::"""
        ...
        # For now, just validate that we can create the complex page
        # TODO: Add performance validation once playwright integration is stable
        assert page_id is not None, "Complex ADF page should be created successfully"
