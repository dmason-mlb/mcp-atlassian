...
class TestConfluencePageOperations:
    ...
    @pytest.mark.anyio
    async def test_get_page_by_title_and_space(
        self, mock_context, mock_confluence_fetcher
    ):
        ...
        with patch(
            ...
            result = await get_page(mock_context, title="My Page", space_key="PERSONAL")
            parsed = json.loads(result)
    ...
    async def test_get_page_not_found(self, mock_context, mock_confluence_fetcher):
        ...
        # Arrange
        mock_confluence_fetcher.get_page.side_effect = MCPAtlassianError(
            "Page not found"
        )
    ...
    @pytest.mark.anyio
    async def test_get_page_without_markdown_conversion(
        self, mock_context, mock_confluence_fetcher
    ):
        ...
        with patch(
            ...
            result = await get_page(
                mock_context, page_id="345678", convert_to_markdown=False
            )
            parsed = json.loads(result)
    ...
    async def test_create_page_success(self, mock_context, mock_confluence_fetcher):
        ...
        with (
            ...
            patch(
                "mcp_atlassian.servers.confluence.pages.check_write_access"
            ) as mock_check_write,
        ):
            mock_get_fetcher.return_value = mock_confluence_fetcher
    ...
    @pytest.mark.anyio
    async def test_create_page_read_only_mode(
        self, mock_context, mock_confluence_fetcher
    ):
...
class TestConfluenceSearch:
    ...
    @pytest.mark.anyio
    async def test_search_with_simple_query(
        self, mock_context, mock_confluence_fetcher
    ):
    ...
    async def test_search_with_cql_query(self, mock_context, mock_confluence_fetcher):
        ...
        with patch(
            ...
            result = await search(
                mock_context, query='type=page AND space=DEV AND title~"API*"'
            )
            parsed = json.loads(result)
...
class TestConfluenceContent:
    ...
    async def test_add_comment(self, mock_context, mock_confluence_fetcher):
        ...
        with (
            ...
            patch(
                "mcp_atlassian.servers.confluence.content.check_write_access"
            ) as mock_check_write,
        ):
            mock_get_fetcher.return_value = mock_confluence_fetcher
            mock_check_write.return_value = None
            ...
            result = await add_comment(
                mock_context, page_id="123456", content="New comment"
            )
            parsed = json.loads(result)
    ...
    async def test_add_label(self, mock_context, mock_confluence_fetcher):
        ...
        with (
            ...
            result = await add_label(mock_context, page_id="123456", name="new-label")
            parsed = json.loads(result)
...
class TestConfluenceErrorHandling:
    ...
    async def test_auth_error_handling(self, mock_context):
        ...
        mock_confluence_fetcher.get_page.side_effect = MCPAtlassianAuthenticationError(
            "Invalid token"
        )
    ...
    async def test_network_error_handling(self, mock_context):
        ...
        # Arrange
        mock_confluence_fetcher = AsyncMock(spec=ConfluenceFetcher)
        mock_confluence_fetcher.search.side_effect = ConnectionError(
            "Network unreachable"
        )
...
class TestConfluenceToolRegistration:
    ...
    def test_confluence_tools_have_correct_tags(self):
        ...
        for tool_name, tool in all_tools.items():
            ...
            if any(
                action in tool_name for action in ["create", "update", "delete", "add"]
            ):
