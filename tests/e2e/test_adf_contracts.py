...
class TestADFMCPContracts:
    ...
    @pytest.mark.asyncio
    async def test_jira_issue_adf_mcp_contract(self, mcp_client, atlassian_stub, sample_test_data):
        ...
        if isinstance(description_field, dict):
            assert "type" in description_field, "Description should be ADF JSON with 'type' field"
            assert description_field["type"] == "doc", "ADF root should be document type"
            assert "content" in description_field, "ADF should have content array"
        
        # Validate ADF content contains expected elements
        self._validate_adf_elements_in_payload(description_field, [
            "heading", "panel", "codeBlock", "status"
        ])
    
    @pytest.mark.asyncio
    async def test_confluence_page_adf_mcp_contract(self, mcp_client, atlassian_stub, sample_test_data):
        ...
        if "atlas_doc_format" in body_field:
            ...
            self._validate_adf_elements_in_payload(adf_value, [
                "heading", "panel", "codeBlock", "status", "date"
            ])
    
    @pytest.mark.asyncio
    async def test_jira_comment_adf_mcp_contract(self, mcp_client, atlassian_stub, sample_test_data):
...
:::"""
        ...
        if isinstance(comment_body, dict):
            ...
            self._validate_adf_elements_in_payload(comment_body, ["heading", "codeBlock", "status"])
...
Deployment: {status:color=blue}Scheduled{/status}"""
        ...
        if isinstance(description, dict):
            ...
            assert len(status_nodes) >= 4, f"Should find 4 status elements, found {len(status_nodes)}"
            ...
            for status_node in status_nodes:
                ...
                assert "color" in status_node["attrs"], "Status should have color attribute"
                assert status_node["attrs"]["color"] in ["green", "red", "yellow", "blue"], \
...
:::"""
        ...
        # Validate API payload
        call_body = atlassian_stub.call_log[0][2]
        body_content = call_body.get("body", {}).get("atlas_doc_format", {}).get("value", {})
        
        if isinstance(body_content, dict):
            # Find panel nodes
            panel_nodes = self._find_adf_nodes_by_type(body_content, "panel")
            assert len(panel_nodes) >= 4, f"Should find 4 panels, found {len(panel_nodes)}"
            
            # Validate panel types
            panel_types = [node["attrs"]["panelType"] for node in panel_nodes if "attrs" in node]
    ...
    async def test_adf_code_block_contract(self, mcp_client, atlassian_stub):
        ...
        # Stub response
        expected_issue = {"key": "TEST-CODE", "fields": {"description": {"type": "doc", "content": []}}}
        atlassian_stub.stub_jira_create_issue(project_key, expected_issue)
...
```"""
        ...
        if isinstance(description, dict):
            ...
            assert len(code_blocks) >= 3, f"Should find 3 code blocks, found {len(code_blocks)}"
            
            # Validate language attributes
            languages = [block["attrs"]["language"] for block in code_blocks if "attrs" in block]
    ...
    def _validate_adf_elements_in_payload(self, adf_payload: Dict, expected_types: list):
