...
class TestADFRegressionJira:
    ...
    def setup_method(self):
        ...
        self.cloud_preprocessor = JiraPreprocessor(base_url="https://company.atlassian.net")
    ...
    def test_legacy_wiki_markup_conversion_unchanged(self):
        ...
        test_cases = [
            ...
            ("***Bold italic***", "*Bold italic*"),  # Actual behavior: nested bold/italic becomes just bold

            # Lists
            ("- List item 1\n- List item 2", "* List item 1\n* List item 2"),
            ("1. First item\n2. Second item", "1. First item\n2. Second item"),  # Ordered lists maintain markdown format
        ...
        for markdown_input, expected_wiki in test_cases:
            ...
            result = self.server_preprocessor.markdown_to_jira(markdown_input, enable_adf=False)
            
            # Should return string (wiki markup) not dict (ADF)
            assert isinstance(result, str), f"Expected string for: {markdown_input}"
            assert expected_wiki in result, f"Expected '{expected_wiki}' in result '{result}' for input: {markdown_input}"
...
[Link](https://example.com)"""
        ...
        result = self.server_preprocessor.markdown_to_jira(test_markdown, enable_adf=True)
...
This is a **bold** test with *italic* text."""
        ...
        result = self.cloud_preprocessor.markdown_to_jira(test_markdown, enable_adf=True)
    ...
    def test_adf_disabled_fallback_behavior(self):
        ...
        server_result = self.server_preprocessor.markdown_to_jira(test_markdown, enable_adf=False)
        cloud_result = self.cloud_preprocessor.markdown_to_jira(test_markdown, enable_adf=False)
    ...
    def test_empty_input_handling_unchanged(self):
        ...
        for empty_input in empty_inputs:
            # With ADF disabled
            server_result = self.server_preprocessor.markdown_to_jira(empty_input, enable_adf=False)
            ...
            server_adf_result = self.server_preprocessor.markdown_to_jira(empty_input, enable_adf=True)
            ...
            cloud_adf_result = self.cloud_preprocessor.markdown_to_jira(empty_input, enable_adf=True)
    ...
    def test_jira_to_markdown_conversion_unchanged(self):
        ...
        jira_markup_cases = [
            ...
            # Lists  
            ("* Item 1\n* Item 2", "- Item 1\n- Item 2"),
            ("# Item 1\n# Item 2", "1. Item 1\n1. Item 2"),  # Jira ordered lists use # for each item
        ...
        for jira_input, expected_markdown in jira_markup_cases:
            ...
            assert expected_markdown in result, f"Expected '{expected_markdown}' in result '{result}' for input: {jira_input}"
    ...
    def test_error_handling_fallback_unchanged(self):
        ...
        with patch.object(self.server_preprocessor.format_router, 'convert_markdown') as mock_convert:
            mock_convert.side_effect = Exception("Mock conversion error")
            
            result = self.server_preprocessor.markdown_to_jira("**bold** text", enable_adf=True)
...
class TestADFRegressionConfluence:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        self.server_preprocessor = ConfluencePreprocessor(base_url="https://confluence.company.com")
        self.cloud_preprocessor = ConfluencePreprocessor(base_url="https://company.atlassian.net")
...
- List item 2"""
        ...
        result = self.server_preprocessor.markdown_to_confluence(test_markdown, enable_adf=True)
...
This is a **bold** test with *italic* text."""
        ...
        result = self.cloud_preprocessor.markdown_to_confluence(test_markdown, enable_adf=True)
    ...
    def test_error_handling_fallback_unchanged(self):
        ...
        with patch.object(self.server_preprocessor.format_router, 'convert_markdown') as mock_convert:
            ...
            result = self.server_preprocessor.markdown_to_confluence("**bold** text", enable_adf=True)
...
class TestADFRegressionGeneral:
    ...
    def test_all_existing_tests_still_pass(self):
        ...
        confluence_processor = ConfluencePreprocessor(base_url="https://confluence.company.com")
        
        # Basic functionality should work
        jira_result = jira_processor.markdown_to_jira("**test**", enable_adf=False)
        assert "*test*" in jira_result
        
        confluence_result = confluence_processor.markdown_to_confluence_storage("**test**")
    ...
    def test_no_memory_leaks_or_performance_degradation(self):
        ...
        assert total_time < 1.0, f"Processing 50 iterations took {total_time:.3f}s, should be < 1.0s"
