...
import pytest
from validators import DeploymentType, validate_confluence_response
...
class TestConfluencePageCreation(MCPConfluenceTest):
    ...
    async def test_create_page_with_adf_content(
        ...
        validation = validate_confluence_response(
            result, "create", DeploymentType.CLOUD
        )
    ...
    async def test_create_page_with_wiki_markup(
        self, mcp_client, test_config, test_data_manager
    ):
...
"""
    ...
    async def test_create_page_with_table(
        self, mcp_client, test_config, test_data_manager
    ):
...
class TestConfluencePageUpdates(MCPConfluenceTest):
    ...
    async def test_update_page_with_complex_content(
        self, mcp_client, test_config, test_data_manager
    ):
    ...
    async def test_update_nonexistent_page(self, mcp_client, test_config):
        ...
        # The MCP tool returns an empty dict {} for non-existent pages
        # This is the expected behavior - the operation completes without error
        # but returns no content, indicating the page doesn't exist
        assert isinstance(result, dict), f"Expected dict result, got: {type(result)}"
        assert result == {} or len(result) == 0, (
            f"Expected empty result for non-existent page, got: {result}"
        )
...
class TestConfluenceLabels(MCPConfluenceTest):
    ...
    async def test_add_single_label(self, mcp_client, test_config, test_data_manager):
        ...
        label_result = await mcp_client.add_confluence_label(
            page_id=page_id, name=test_label
        )
    ...
    async def test_add_multiple_labels(
        ...
        # Add multiple labels
        labels = [test_config["test_label"], "documentation", "testing", "automation"]

        for label in labels:
            label_result = await mcp_client.add_confluence_label(
                page_id=page_id, name=label
            )
            self.assert_success_response(label_result)
    ...
    async def test_add_label_to_nonexistent_page(self, mcp_client, test_config):
        ...
        result = await mcp_client.add_confluence_label(
            page_id="999999999", name="should-fail"
        )
...
class TestConfluencePageHierarchy(MCPConfluenceTest):
    ...
    async def test_create_page_hierarchy(
        self, mcp_client, test_config, test_data_manager
    ):
    ...
    async def test_move_page_hierarchy(
        self, mcp_client, test_config, test_data_manager
    ):
        ...
        parent2_result = await self.create_test_page(
            mcp_client, test_config, content="# New Parent\n\nNew parent page."
        )
...
class TestConfluenceErrorHandling(MCPConfluenceTest):
    ...
    async def test_create_duplicate_title_in_space(
        self, mcp_client, test_config, test_data_manager
    ):
...
class TestConfluenceBulkOperations(MCPConfluenceTest):
    ...
    async def test_create_multiple_pages(
        self, mcp_client, test_config, test_data_manager
    ):
    ...
    async def test_batch_comments(self, mcp_client, test_config, test_data_manager):
        ...
        for i, comment_text in enumerate(comments):
            comment_result = await mcp_client.add_confluence_comment(
                page_id=page_id, content=f"{comment_text} (#{i + 1})"
            )
            self.assert_success_response(comment_result)
