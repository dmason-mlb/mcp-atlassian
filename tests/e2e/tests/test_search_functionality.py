...
from .base_test import MCPBaseTest, MCPConfluenceTest, MCPJiraTest
...
class TestJiraSearch(MCPJiraTest):
    ...
    async def test_jira_basic_search(self, mcp_client, test_config):
        ...
        assert issue1_key in issue_keys, (
            f"Issue {issue1_key} not found in search results"
        )
        assert issue2_key in issue_keys, (
            f"Issue {issue2_key} not found in search results"
        )
    ...
    async def test_jira_jql_query_validation(self, mcp_client, test_config):
        ...
        for test_case in jql_tests:
            search_result = await mcp_client.jira_search(jql=test_case["jql"], limit=5)
            ...
            if test_case["should_find"]:
                assert issue_key in found_keys, (
                    f"JQL test '{test_case['description']}' should find issue {issue_key}"
                )
            else:
                assert issue_key not in found_keys, (
                    f"JQL test '{test_case['description']}' should not find issue {issue_key}"
                )
    ...
    async def test_jira_field_search(self, mcp_client, test_config):
        ...
        field_result = await mcp_client.jira_search_fields(keyword="summary", limit=10)
    ...
    async def test_jira_search_with_pagination(self, mcp_client, test_config):
        ...
        if page1_issues and page2_issues:
            assert not (page1_keys & page2_keys), (
                f"Pages should not have overlapping issues. Page1: {page1_keys}, Page2: {page2_keys}"
            )

        # We should have created enough issues to fill both pages
        total_found = len(page1_keys | page2_keys)
        assert total_found >= 6, (
            f"Should find most of our {len(issue_keys)} test issues, found {total_found}"
        )
    ...
    async def test_jira_advanced_search_filters(self, mcp_client, test_config):
        ...
        today_search = await mcp_client.jira_search(
            jql=f"project = {test_config['jira_project']} AND created >= -1d", limit=10
        )
...
class TestConfluenceSearch(MCPConfluenceTest):
    ...
    async def test_confluence_basic_search(self, mcp_client, test_config):
        ...
        search_result = await mcp_client.confluence_search(
            query="Search Test Page", limit=10
        )
        ...
        assert len(results) >= 2, (
            f"Should find at least 2 test pages, found: {len(results)}"
        )

        # Verify both pages are found
        found_ids = []
        for result in results:
            page_id = self.extract_value(result, "content", "id") or self.extract_value(
                result, "id"
            )
            if page_id:
    ...
    async def test_confluence_cql_search(self, mcp_client, test_config):
        ...
        await self.wait_for_confluence_page_in_search(
            mcp_client, page_title, timeout=30
        )
        ...
        for test_case in cql_tests:
            ...
            found_ids = []
            for result in results:
                found_id = self.extract_value(
                    result, "content", "id"
                ) or self.extract_value(result, "id")
                if found_id:
                    found_ids.append(str(found_id))

            if test_case["should_find"]:
                assert page_id in found_ids, (
                    f"CQL test '{test_case['description']}' should find page {page_id}"
                )
            else:
                assert page_id not in found_ids, (
                    f"CQL test '{test_case['description']}' should not find page {page_id}"
                )
...
"""
        ...
        content_page = await self.create_test_page(
            mcp_client, test_config, content_format="markdown", content=rich_content
        )
        ...
        for search_term in content_searches:
            ...
            assert page_id in found_ids, (
                f"Content search for '{search_term}' should find page {page_id}"
            )
    ...
    async def test_confluence_search_pagination(self, mcp_client, test_config):
        ...
        assert len(found_test_pages) > 0, (
            "Should find at least one test page in search results"
        )
...
""",
        ...
        confluence_results = (
            self.extract_value(confluence_search, "results") or confluence_search
        )
        ...
        assert page_id in confluence_ids, (
            f"Confluence search should find page {page_id}"
        )
    ...
    async def test_search_error_handling(self, mcp_client, test_config):
        ...
        empty_search = await mcp_client.confluence_search(query="", limit=5)
    ...
    async def test_search_performance_limits(self, mcp_client, test_config):
        ...
        large_limit_search = await mcp_client.jira_search(
            jql=f"project = {test_config['jira_project']}", limit=100
        )
        ...
        performance_search = await mcp_client.confluence_search(
            query=f"space={test_config['confluence_space']}", limit=20
        )
