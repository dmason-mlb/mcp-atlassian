...
"""
        ...
        for cell in first_row["content"]:
            assert cell["type"] == "tableHeader", f"Expected tableHeader but got {cell['type']}"
        
        # Check subsequent rows contain tableCell
        for row in table["content"][1:]:
            for cell in row["content"]:
                assert cell["type"] == "tableCell", f"Expected tableCell but got {cell['type']}"
...
"""
        ...
        for i, cell in enumerate(first_row["content"]):
            assert cell["type"] == "tableHeader", f"Cell {i} in first row should be tableHeader"
            # Verify content
            para = cell["content"][0]
        ...
        for i, cell in enumerate(second_row["content"]):
            assert cell["type"] == "tableCell", f"Cell {i} in second row should be tableCell"