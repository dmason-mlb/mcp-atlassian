...
import pytest
from validators import DeploymentType, validate_jira_response
...
class TestJiraIssueCreation(MCPJiraTest):
    ...
    async def test_create_story_with_assignee(
        self, mcp_client, test_config, test_data_manager
    ):
    ...
    async def test_create_bug_with_priority(
        self, mcp_client, test_config, test_data_manager
    ):
    ...
    async def test_create_issue_with_labels(
        ...
        result = await mcp_client.create_jira_issue(
            ...
            summary=summary,
            issue_type="Task",
            additional_fields={"labels": [test_label, "automation", "mcp-test"]},
        )
    ...
    async def test_create_issue_invalid_project(self, mcp_client, test_config):
        ...
        result = await mcp_client.create_jira_issue(
            project_key="INVALID", summary="Should Fail", issue_type="Task"
        )
...
class TestJiraIssueUpdates(MCPJiraTest):
    ...
    async def test_update_issue_summary(
        self, mcp_client, test_config, test_data_manager
    ):
        ...
        update_result = await mcp_client.update_jira_issue(
            issue_key=issue_key, fields={"summary": new_summary}
        )
    ...
    async def test_update_issue_description(
        ...
        update_result = await mcp_client.update_jira_issue(
            issue_key=issue_key, fields={"description": new_description}
        )

        self.assert_success_response(update_result)

        # Validate ADF formatting in response
        validation = validate_jira_response(
            update_result, "update", DeploymentType.CLOUD
        )
    ...
    async def test_update_issue_labels(
        ...
        update_result = await mcp_client.update_jira_issue(
            issue_key=issue_key,
            fields={},
            additional_fields={"labels": [test_label, "updated", "test-automation"]},
        )
    ...
    async def test_update_nonexistent_issue(self, mcp_client, test_config):
        ...
        result = await mcp_client.update_jira_issue(
            issue_key="FAKE-99999", fields={"summary": "Should Fail"}
        )
...
class TestJiraComments(MCPJiraTest):
    ...
    async def test_add_basic_comment(self, mcp_client, test_config, test_data_manager):
        ...
        comment_result = await mcp_client.add_jira_comment(
            issue_key=issue_key, comment=comment_text
        )
    ...
    async def test_add_formatted_comment(
        self, mcp_client, test_config, test_data_manager
    ):
...
> This is a blockquote in the comment."""
        ...
        validation = validate_jira_response(
            comment_result, "comment", DeploymentType.CLOUD
        )
        assert validation.is_valid, (
            f"ADF comment validation failed: {validation.errors}"
        )

    async def test_add_comment_with_mentions(
        self, mcp_client, test_config, test_data_manager
    ):
...
class TestJiraTransitions(MCPJiraTest):
    ...
    async def test_transition_with_resolution(
        self, mcp_client, test_config, test_data_manager
    ):
        ...
        for transition in transitions:
            if any(
                word in transition["name"].lower()
                for word in ["done", "resolve", "complete", "close"]
            ):
...
class TestJiraErrorHandling(MCPJiraTest):
    ...
    async def test_update_issue_invalid_fields(
        self, mcp_client, test_config, test_data_manager
    ):
        ...
        result = await mcp_client.update_jira_issue(
            issue_key=issue_key,
            fields={},
            additional_fields={"priority": {"name": "InvalidPriority"}},
        )

        self.assert_error_response(result, r"priority|invalid|not valid")

    async def test_transition_invalid_transition_id(
        self, mcp_client, test_config, test_data_manager
    ):
...
class TestJiraBulkOperations(MCPJiraTest):
    ...
    async def test_create_multiple_issues(
        self, mcp_client, test_config, test_data_manager
    ):
    ...
    async def test_batch_comments(self, mcp_client, test_config, test_data_manager):
        ...
        for i, comment_text in enumerate(comments):
            comment_result = await mcp_client.add_jira_comment(
                issue_key=issue_key, comment=f"{comment_text} (#{i + 1})"
            )
            self.assert_success_response(comment_result)
